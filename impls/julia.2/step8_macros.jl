    ...
    while true
        ...
        elseif ast isa Vector && length(ast) > 0
            ...
            elseif ast[1] == Symbol("fn*")
                fn = (args...) -> EVAL(ast[3], Env(env, ast[2], Any[args...]))
                return Types.MalFunction(ast[3], ast[2], env, fn)
            elseif ast[1] == :quote
                return ast[2]
            elseif ast[1] == :quasiquote
                ast = quasiquote(ast[2])
            elseif ast[1] == :defmacro!
                return env_set!(env, ast[2], Types.asmacro(EVAL(ast[3], env)))
            else
                callable = EVAL(ast[1], env)
                if callable isa Types.MalFunction && callable.ismacro
                    ast = callable.fn(ast[2:end]...)
                else
                    args = [EVAL(member, env) for member in ast[2:end]]
                    if callable isa Types.MalFunction
                        env = Env(callable.env, callable.params, args)
                        ast = callable.ast
                    else
